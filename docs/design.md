...
| `dekopon-broker-host` | Privileged async Wasmtime adapter consuming authorized invocations and exact optional storage grants, linking only versioned Dekopon HTTP/storage/clock imports, and emitting bounded metadata | **Current** library used by the separate broker process |
