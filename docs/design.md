...
- A **harness** is the unprivileged runtime that executes a configured agent's bounded session.
...
## Unreleased harness runtime

[Harness sessions](harness.md) defines current runtime ownership and the remaining integration
limitations. Fresh scoped schemas precede request one. Execution evidence, selected context,
checkpoint receipts, model spend and transport acceptance are separate facts. Controls require
fresh broker admission for each changed dimension; tools and checkpoints confer no authority.
Safe model/effort switches preserve budgets and evidence and discard provider continuation.
Cosmetic progress remains transport-owned and outside history, checkpoints and receipts.
...
parse dekopon-run CLI
  ...
  -> session: list, show, or replay exported transcript records read back from OpenObserve
...
where it composed the published `dekopon-agent` and `dekopon-broker-protocol` crates.
This tree's unreleased clean break requires an explicit out-of-tree client migration; no console
upgrade is claimed here. That is the shape a session-running client takes — a gateway for one
...
| Harness-owned conversation state in the gateway process | Immediate replay uses per-sender generation leases, bounded execution-aware history and separately bounded model context in the unprivileged harness. Optional durable memory is a separate broker-owned provider store: content is namespace-bound and model-hidden on write, omitted from audit/telemetry, and retrieved only on demand. Authorization stays uncached in both mechanisms. |
