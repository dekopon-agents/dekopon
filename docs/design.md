...
| `dekopond` | Chat-transport wakeups, attested routing, bounded agent sessions with no broker authority, credential-free self-inspection, and bounded per-sender conversation history | **Current** unprivileged daemon; a route is one independent session per message unless it opts into `mode: persistent`, and a dedicated gateway UID remains **committed direction** |
...
The published `0.7.0` release retains the local catalog read path introduced in 0.1:
