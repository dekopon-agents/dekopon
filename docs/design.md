...
| `dekopon-brokerd` | Owner-only Unix peer authentication, Cedar authorization, destination-bound credential resolution, replay restoration, provider execution, evidence, durable audit, atomic local checkpoint verification, an explicitly enabled unauthenticated read-only web view, and a separate offline exact-reference OCI provider-manager mode | **Current** privileged process; daemon startup is network-free and managed locks are checked against the exact Wasmtime input buffer; provenance verification, SemVer updates, pruning, and independent remote/signed audit anchoring remain future |
