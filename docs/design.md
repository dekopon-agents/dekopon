...
| `dekopon-agent` | The shared agent session layer: the bounded scripting prompt loop, optional bounded asset/configuration meta tools, the script runtime spending a session-wide capability budget, and a broker-leg facade over the protocol client | **Current**, holding no authority; consumed by `dekopon-run` and `dekopond` |
...
| `dekopond` | Chat-transport wakeups, attested routing, best-effort native in-flight activity, cooperatively cancellable bounded agent sessions with no broker authority, credential-free self-inspection, and bounded per-sender conversation history | **Current** unprivileged daemon; a route is one independent session per message unless it opts into `mode: persistent`, activity is opt-in and begins only after authorization, and a dedicated gateway UID remains **committed direction** |
...
What it does not buy: **policy still cannot bind a path.** Cedar sees the principal, action,
provider, and trusted routing metadata; it does not see provider input, so no statement can say
"this agent may comment on issues in `dekopon-agents/*` only". A credential per agent narrows *who
may use a token*, not *what that token can touch*. The token's own scope at the provider remains the
boundary on which repositories are reachable, and a deployment that needs that boundary must get it
from the token, not from Dekopon.
...
| Credentials bound per capability, overridable per agent | Which *operation* gets a credential is a capability question — the confused deputy is "same component, different operation". Which *credential* an operation presents to a given caller is a separate question, and keying it on the agent reuses the partition routes already make: one agent per team or channel, one token per organization, no capability duplicated. The agent name is broker-derived attested context rather than a caller claim, and a caller with no agent takes the default. |
