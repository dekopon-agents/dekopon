...
| `dekopon-agent` | The shared agent session layer: the bounded scripting prompt loop, optional bounded asset/configuration/image-generation meta tools, the script runtime spending a session-wide capability budget, and a broker-leg facade over the protocol client | **Current**, holding no authority; consumed by `dekopon-run` and `dekopond` in this tree, and by out-of-tree clients such as `dekopon-console` |
