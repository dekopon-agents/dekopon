...
`gh` is the same idea with a GitHub-CLI-shaped vocabulary. It parses subcommands (`gh pr view 7 -R owner/repo`, `gh pr review 7 -R owner/repo --comment --body text --expected-head-sha abc123`, `gh content view <path> -R owner/repo`, `gh issue`, `gh repo`, `gh branch`, `gh commit`, `gh user`) and dispatches each one to the correspondingly named `gh.*` capability — `gh pr view` to `gh.pull-request.read`, `gh pr review --comment` to `gh.pull-request.comment` — through the same budgeted capability seam. All three review events accept `--expected-head-sha`, so a comment, approval, or change request can refuse instead of landing on commits its caller did not inspect. A subcommand whose capability is not granted reports the missing capability by name with exit code `127`; flags that would change what a command means (`--json`, `--jq`, `--web`, `--checkout`) are rejected with guidance rather than ignored, and `gh api` is refused outright because raw passthrough would bypass per-capability authorization. Every `gh.*` capability also remains directly invocable as a command word (`gh.pull-request.read --owner o --repo r --number 7`); the builtin is ergonomics, not authority.
