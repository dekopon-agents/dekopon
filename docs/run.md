...
- `--provider <COMPONENT>` (repeatable; a file, or a directory of `*.wasm`) supplies read-only, import-free components that run a script the recording cannot answer, in direct mode under the same `--compile-cache` (or `DEKOPON_RUN_COMPILE_CACHE`), Wasm bounds (`--max-memory-bytes`, `--max-input-bytes`, `--max-output-bytes`, `--fuel`, `--timeout-ms`), and interpreter bounds (the `--shell-*` flags) as `prompt`. Nothing is loaded unless the flag is passed, so the default replay is provably effect-free; with it, a diverging script can compute but never reach the network, and `curl` has no capability to assemble for. `--shell-max-capability-calls` bounds the whole replayed session as it does in `prompt`; a script answered from the recording spends none of it.
