...
provider broker leg. Memory receipts do not imply crash durability. Stdout delivery finalizes on
what the write actually did: a complete write and flush is `Accepted`, carrying the exact bytes;
a failure while flushing is `Unknown`, because the reader may already hold them; a failure before
any byte left is `Failed`; and a short write is `Partial`.
...
`STARTED` is the earliest accounted turn, RFC 3339 UTC to the second; `TURNS` the highest turn accounted; `TOKENS` the sum only when every accounted call reports `usage.total_tokens` and addition fits, otherwise `-`; `OUTCOME` is `failed` when any accounted call's `outcome` was `failed`, `cancelled`, or `abandoned`, otherwise `answered` or `no-answer` by whether the last turn carried an answer; `SERVICE` is the records' `service_name`, or `-`. An empty window prints `(no sessions in the window)`. `--json` prints the same rows as an array of `{traceId, service, startedUs, endedUs, modelTurns, totalTokens, failed, answered}`, with microsecond epoch timestamps and `null` where nothing was reported.

### `session show`

`dekopon-run session show (--trace-id <TRACE_ID> | --from-file <PATH>) [--json]` reconstructs one transcript. Exactly one source is required; naming both or neither is a usage error. `--trace-id` fetches every record of that trace from the receiver; `--from-file` reads a transcript an earlier `session show --json` printed and touches no backend, so the receiver flags are not needed. Portable request fragments are retained in turn order: `delta` appends within one context revision and a later `full` replaces it. Assistant turns come from their own answer records; rebuilt tool results use host job/logical-call coordinates, not provider-ID reuse or answer-text matching. Usage and duration come independently from accounting records. Records may arrive in any order; a transcript that is not the shape the loop writes is refused as malformed, naming the event.

The text rendering shows `trace:`, each leading `system:` message, the exchanges a persistent route replayed as `user (earlier):` and `assistant (earlier):`, the `user:` prompt, then `turn N:` for each model turn — with `[<ms> ms, <tokens> tokens]` when the accounting record carried them — holding its `assistant:` text and every `script:` with its `output:` (`(not recorded)` when the session ended before answering it; a call that was not a script shows as `tool <name>:` with its arguments), and finally `answer:`, or `answer: (none recorded)`. `--json` prints the exact document `replay --from-file` reads back. That document declares its own shape in `version`, which is checked before any other field: a file omitting it is read as version `1`, and one declaring anything else is refused naming the version it declared. An unknown top-level key is refused too, so a file edited into a shape this build does not understand fails at the boundary rather than replaying with a field silently ignored:
...
Current exports additionally retain `contexts`: `{turn, revision, scope, messages}` portable full/delta fragments. Replay uses the first request's non-system history, including matched tool groups and labelled execution/delivery summaries, then the recorded prompt exactly once. Later full revisions appear under `context revision N (turn M):` in text output and never add executions or accounting calls. An excerpt without a complete recorded script result is not used to invent output or an exit code. `history` remains a lossless text-pair projection where possible and the fallback for historical files without `contexts`; `usage` may also carry `cachedInputTokens` and `reasoningOutputTokens`; a field that was not recorded is omitted. The file is the recording, so it can be kept, edited by hand, and replayed with no backend in the loop.
...
The report (`--json`) is `{traceId, recorded, replayed, divergence, suggestions, droppedHistoryTurns, error}`. `recorded` and `replayed` are each `{modelTurns, scripts, answer, usage}` — the scripts in order, the final answer or `null`, and unknown-aware usage from independent accounting calls, including failures and images. `modelTurns` is `null`, and renders as `-`, when nothing in the recording says how many chat calls it made: a `calls` list that is present but names no chat call — empty, or image calls only, which is what a truncated page set or a hand-edited recording produces — is unknown, never zero. A file written before independent call accounting existed carries no `calls` key at all and falls back to its answered turns; a reconstruction always writes the key, so a current trace whose accounting rows the receiver did not return reads as unknown rather than as a confident count of a different quantity. `divergence` is `null` or `{turn, script, unusedRecordedScripts, handling}`: the replayed turn that wrote the script, the script, and the recorded scripts not yet consumed. `suggestions` lists what `--suggestions` recorded. `droppedHistoryTurns` is how many recorded earlier exchanges did not fit the replay's retention window (128 turns, 1 MiB); it is `0` for every ordinary recording, and when it is not, the text rendering adds a `history:` line, because the replayed model saw less than the recorded one did. `error` is `null` unless the replayed session failed for a reason other than a divergence stop — a model failure, `--max-steps` exhausted, a malformed tool call — in which case the report is still printed and the exit code is `1`. The text rendering summarizes both sessions, states `divergence: none` or `divergence: turn N (stopped there | ran live), K recorded script(s) unused` with the script, compares scripts index by index as `script N (same | differs | recorded only | replayed only):` — the recorded text, and the replayed text when it is not the same — and ends with `answer (recorded):` and `answer (replayed):`:
