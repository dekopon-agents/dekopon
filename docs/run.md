...
A **provider command word** is the same idea, supplied by the component rather than compiled in. A provider declares `commandWords` in its manifest and exports `resolve-command`, and the shell resolves those words after builtins and before capability fallback: the component rewrites the argv into a capability proposal, and that proposal is authorized on exactly the path a direct `some.capability --flag value` call takes — same budget, same denial, same telemetry. Rewriting proposes; it grants nothing. A word whose capability is not granted in this session reports the missing capability by name with exit code `127`, so "not configured" never reads as "does not exist".

The GitHub provider is the worked example, and it lives in [its own repository](https://github.com/dekopon-agents/dekopon-provider-gh) rather than here. It contributes the `gh` word and the GitHub-CLI vocabulary behind it (`gh pr view 7 -R owner/repo`, `gh pr review 7 -R owner/repo --approve`); flags that would change what a command means are rejected with guidance rather than ignored, and `gh api` is refused outright because raw passthrough would bypass per-capability authorization. None of that is knowledge this repository carries: drop the component into a provider directory and the word appears, remove it and the word goes.
