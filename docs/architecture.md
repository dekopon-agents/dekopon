...
## Published 0.5.0 foundation

The published `0.5.0` release changes no crate boundary. It retains the two deliberately separate synchronous execution surfaces introduced in 0.1, the privileged asynchronous host, authorization/evidence/audit libraries, separately deployed authenticated Unix broker, sandboxed one-tool scripting surface, and cross-process OpenTelemetry added in 0.2, and the unprivileged agent daemon `dekopond` added in 0.3, which connects to chat services and submits attested on-behalf-of proposals to the broker. What 0.4 adds is distribution — a container image assembled from the release archives, a Helm chart that runs both daemons as one pod, and a Homebrew tap — none of which moves a responsibility between processes. Explicit `dekopon-run broker` commands are unprivileged clients. The operator CLI is still not broker- or daemon-integrated. `dekopon-run` exports correlated OTLP traces and logs, while `dekopon-brokerd` and `dekopond` independently export their own trace legs and write structured JSON logs to stdout for an external collector; collecting Kubernetes node telemetry or unrelated process logs remains separate deployment work.

Current development after that release adds one crate without moving authority: `dekopon-webui`, a GET-only operational renderer embedded in the broker behind an explicit TCP bind. The gateway remains the owner of agent catalog/model accounting and publishes only bounded non-authoritative summaries; the broker remains the owner of provider runtime observations. The crate list below describes this current tree, not the immutable 0.5.0 artifacts.
...
- `dekopon-agent`: the shared agent session layer — the bounded one-tool prompt loop, the script runtime that spends a session-wide capability budget on fresh interpreters, composite direct-then-broker capability dispatch, and a synchronous broker-leg facade over the protocol client. It holds no authority and depends only on the client half of the broker protocol; `dekopon-run` and `dekopond` are both consumers.
