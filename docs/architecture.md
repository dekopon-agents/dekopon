...
- `dekopon-config`: discovery, parsing, duplicate detection, and reference validation, plus the bounded in-memory `Skill` loader for `SKILL.md` directories, which the catalog runs at load time and `dekopon-run prompt --skill` runs before any model call, so a session never touches the filesystem for one.
