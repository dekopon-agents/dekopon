...
## Published 0.6.0 foundation

The published `0.6.0` release adds one crate without moving authority: `dekopon-webui`, a GET-only operational renderer embedded in the broker behind an explicit TCP bind. The gateway remains the owner of agent catalog/model accounting and publishes only bounded non-authoritative summaries; the broker remains the owner of provider runtime observations. The UI cannot influence identity, policy, execution constraints, credentials, provider routing, evidence, replay, or durable audit.

The release otherwise retains the two deliberately separate synchronous execution surfaces introduced in 0.1, the privileged asynchronous host, authorization/evidence/audit libraries, separately deployed authenticated Unix broker, sandboxed one-tool scripting surface, and cross-process OpenTelemetry added in 0.2, and the unprivileged agent daemon `dekopond` added in 0.3, which connects to chat services and submits attested on-behalf-of proposals to the broker. Version 0.4 added distribution through release archives, a container image, a Helm chart, and a Homebrew tap; 0.5 added bounded chat attachments. None moved a responsibility between processes. Explicit `dekopon-run broker` commands remain unprivileged clients, and the operator CLI is still not broker- or daemon-integrated.
