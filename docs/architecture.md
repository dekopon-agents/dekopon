...
## Published 0.8.0 foundation

The published `0.8.0` release adds Discord Gateway v10 as a `dekopond` chat transport without moving authority. The daemon uses non-privileged message intents, authenticated structured mentions for guild wakeups, resumable outbound Gateway sessions, bounded no-ping REST replies, and tokenless host-validated CDN downloads through the existing lazy attachment path. Discord users become canonical `discord.<user id>` subjects that the broker still maps and authorizes; the gateway gains no policy, provider credential, or effect path.
