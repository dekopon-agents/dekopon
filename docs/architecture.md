...
## Published 0.9.0 foundation

The published `0.9.0` release adds opt-in transport-native in-flight activity after fresh authorization: Slack Agent sessions use `processing`/`active` and authenticated cooperative Stop events with a fixed owned-reaction fallback, Discord uses typing leases, and Telegram uses topic-aware chat actions. Activity targets come only from authenticated transport envelopes, cosmetic failures never alter the durable reply, and cooperative Stop cannot roll back a model request or provider effect already in progress. The release also carries an unsupported, mock-only Skylight provider exploration that is absent from default catalogs, images, policies, and deployments.

The release otherwise retains the two deliberately separate synchronous execution surfaces introduced in 0.1, the privileged asynchronous host, authorization/evidence/audit libraries, separately deployed authenticated Unix broker, sandboxed one-tool scripting surface, and cross-process OpenTelemetry added in 0.2, and the unprivileged agent daemon `dekopond` added in 0.3, which connects to chat services and submits attested on-behalf-of proposals to the broker. Version 0.4 added distribution through release archives, a container image, a Helm chart, and a Homebrew tap; 0.5 added bounded chat attachments; 0.6 added `dekopon-webui`, a GET-only operational renderer embedded in the broker behind an explicit TCP bind; 0.7 added credential-free gateway self-inspection; 0.8 added Discord Gateway transport; and 0.8.1 raised the per-turn tool-call guard from four to ten. None moved a responsibility between processes. Explicit `dekopon-run broker` commands remain unprivileged clients, and the operator CLI is still not broker- or daemon-integrated.
