...
## Unreleased session-runtime boundary

`dekopon-harness` replaces the published `dekopon-agent` name without compatibility wrappers.
`SessionEngine` owns bootstrap, context/history selection, execution observations, bounded memory
checkpoints, strict token accounting and safe-boundary model/effort application. The gateway still
owns ingress/routing/admission, cached model clients, Stop and accepted delivery; only broker/policy
own Cedar decisions. `dekopon-model` requires attempt recording before decoding chat/image results.
`dekopon-core` owns configured-model IDs, opaque control coordinates and explicit effort values.
The local control protocol is now `v1alpha3`; the published 0.12.0 description above remains history.
No provider WIT or privilege dependency changes accompany the harness. See [the runtime contract](harness.md),
including the remaining portable multi-revision replay integration gap.
