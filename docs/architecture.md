...
## Published 0.10.0 foundation

The published `0.10.0` release adds a text-only Meta WhatsApp Cloud API gateway transport (signed bounded webhook, canonical `whatsapp.<wa_id>` subjects), opt-in route-scoped OpenAI image generation with bounded PNG replies, and broker-owned namespace-bound provider storage with strict quotas, transactional JSONL, and the optional generated `memory-chat` provider. It also lands a deep-review hardening pass across the workspace: every broker failure carries a diagnosable cause, the authorization and audit hot paths do their work once, provider compilation is parallel and cacheable, and three recurring failure classes are enforced as deny-by-default clippy lints. The release still carries the unsupported, mock-only Skylight provider exploration, absent from default catalogs, images, policies, and deployments.

The release otherwise retains the two deliberately separate synchronous execution surfaces introduced in 0.1, the privileged asynchronous host, authorization/evidence/audit libraries, separately deployed authenticated Unix broker, sandboxed one-tool scripting surface, and cross-process OpenTelemetry added in 0.2, and the unprivileged agent daemon `dekopond` added in 0.3, which connects to chat services and submits attested on-behalf-of proposals to the broker. Version 0.4 added distribution through release archives, a container image, a Helm chart, and a Homebrew tap; 0.5 added bounded chat attachments; 0.6 added `dekopon-webui`, a GET-only operational renderer embedded in the broker behind an explicit TCP bind; 0.7 added credential-free gateway self-inspection; 0.8 added Discord Gateway transport; 0.8.1 raised the per-turn tool-call guard from four to ten; and 0.9 added opt-in transport-native in-flight activity with authenticated cooperative Stop. None moved a responsibility between processes. Explicit `dekopon-run broker` commands remain unprivileged clients, and the operator CLI is still not broker- or daemon-integrated.
...
## Unreleased Slack Agent continuation boundary
