...
opted in: `dekopon-run prompt --suggestions` or
...
provider command word a direct-mode script runs (`shell` and `prompt`), nested under the command's `shell.command` span; it stays non-interruptible because
