...
`context.revision` and explicit full/delta scope. The current replay reader refuses a recording
with a later full context rebuild rather than silently concatenating incompatible revisions;
portable multi-revision replay remains an integration limitation.
...
The current replay reader safely refuses such multi-revision recordings pending portable
reconstruction. Simple full-plus-delta recordings remain readable.
...
- `session list [--since 7d] [--limit 50] [--json]` reads `accounting.model.call` records and
  ...
  second), `TURNS` (the highest `model.turn`), `TOKENS` (`usage.total_tokens` summed only when every call reports it and arithmetic fits, `-` otherwise), `OUTCOME` (`failed` when any turn was accounted `failed`,
