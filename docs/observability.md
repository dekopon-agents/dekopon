...
Each duplicates span fields by design: the span answers "why was this request slow", the accounting
record answers "how many did we make last month". Image accounting carries the generated byte
count, never the generation prompt or the PNG bytes. None substitutes for broker audit, the only
...
The prompt loop's spans (`prompt.session`, `prompt.model_turn`, `prompt.script`,
`prompt.image_generation`, `shell.script`, `shell.command`) nest under `gateway.session`, and
`broker.invocation` joins the same trace through the proposal's `traceParent` — so one trace reads
from "a person asked something in Slack" to "a provider made an HTTP call". The image span carries
turn and tool indexes and a success byte count, never prompt or PNG content. `prompt.asset_fetch`
joins them whenever a model opens an attachment: one span per fetch, carrying the asset number the
conversation referred to and the turn and tool-call index that asked for it, never the file's name
or bytes. It is gateway-only, because only a gateway session offers the asset tool.
...
A `Redacted` value renders its marker in either payload mode.
