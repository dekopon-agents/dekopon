...
This document follows one agent from a session that went badly to a catalog change that fixes it. It explains the two mechanisms Dekopon gives an operator for that — skills and `suggest_improvement` — how they compose, and what is deliberately not built.
