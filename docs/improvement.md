...
- An embedder of `dekopon-harness` receives them as `SessionExit.suggestions`, already written to telemetry by the time they arrive.
...
- **No durable suggestion store.** A suggestion lives in the telemetry record that carries it and, for the process that ran the session, in `SessionExit.suggestions` until that process exits. Nothing keeps a queue of pending suggestions, marks one applied, or reads yesterday's back into today's session. The telemetry backend already is the store, with the retention, access control, and full-text search an operator configured for the rest of the sink; a second store would be one more place model-authored text lives and one more thing to redact, back up, and expire.
