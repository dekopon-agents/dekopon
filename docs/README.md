...
The unreleased [harness session contract](harness.md) covers the clean-break runtime APIs,
execution-aware history and job state, model controls, strict accounting and remaining integration
gaps. It complements the ownership overview rather than changing broker authority.
...
| Broker-mediated provider storage (`jsonl` or `durable-files`) | [`dekopon-provider-storage`](../crates/dekopon-provider-storage/README.md), [`dekopon-provider-sdk-testkit`](../crates/dekopon-provider-sdk-testkit/README.md), [`dekopon-storage-host`](../crates/dekopon-storage-host/README.md), and [`broker-http.md` § Storage is a sibling privileged host interface](broker-http.md#storage-is-a-sibling-privileged-host-interface) |
...
| The session engine: bootstrap, history and context bounds, job state, accounting, model/effort controls, or dispatch activity | [`harness.md`](harness.md) | Records the unprivileged runtime's contract, its bounds, and what its live job state is not. |
