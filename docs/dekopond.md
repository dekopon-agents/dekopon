...
up to a sixty-second ceiling rather than being refused, and only a sender whose own wait would
exceed that ceiling is refused, with the remaining backoff named. Unknown creation never retries. Platform pacing can
...
drains the activity workers after the sessions and inside the same `shutdownGraceMs`, so a
SIGTERM does not strand a progress message. Late creates are cleaned by
