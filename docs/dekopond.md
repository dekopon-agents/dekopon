...
undroppable seal. There are at most 128 active/cleanup/quarantined target leases and 16 concurrent
cosmetic requests, each with a two-second deadline and 64-KiB response ceiling. Slack updates are
at most once per two seconds and cosmetics share 30 requests per rolling minute. Startup rejects
duplicate authenticated endpoint/team/bot installations. Terminal and cosmetic posts share bounded
one-second channel slots; pending final sends shed new cosmetics without holding any cosmetic I/O
lock. A final post retries only an explicit HTTP 429, once, honoring Retry-After of at most five
seconds within a ten-second pacing budget. Unknown creation never retries. Platform pacing can
delay acceptance; sealing does not wait on cosmetic I/O already issued. Stop, failure,
decline, partial/unknown delivery and owner drop all queue cleanup. Late creates are cleaned by
the original owner; a shared native target cannot start another generation before cleanup, and
uncertain native ordering quarantines that target until restart. Cleanup permits two attempts per
...
bounds. See [harness.md](harness.md) for exact ceilings, checkpoint semantics and known gaps.
