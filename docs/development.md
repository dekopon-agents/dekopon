...
| Broker local protocol/client | `crates/dekopon-broker-protocol/src/lib.rs`; control proposals and their bounds in `control.rs` | Inline strict framing, deadline, authority-omission, socket-metadata, and peer-UID tests, plus control-proposal tests in `crates/dekopon-broker-protocol/src/control/tests.rs` |
| Authenticated Unix broker service, private secret sources, and offline provider manager | `crates/dekopon-brokerd/src/`; strict public-DRN/private-map adapters in `secrets.rs`; provider set/lock, bounded OCI transport, content store, and lifecycle commands in `provider_manager.rs` | Inline strict-config/socket/CLI tests; secret-map aggregate validation, strict JSON/YAML projection, secure-file and mock-backed 1Password/Vault/AWS/GCP/Azure/Kubernetes adapters; local mock-registry resolution, locked-sync, offline list/verify, atomic-activation, and blob-hygiene tests; plus `crates/dekopon-brokerd/tests/server.rs` mapped/unmapped-peer, informational reporting, real HTTP listener, end-to-end invocation, clean-shutdown, and restart-replay tests with model/effort control coverage in `crates/dekopon-brokerd/tests/server/controls.rs`, and `crates/dekopon-brokerd/tests/examples.rs` pinning `examples/conditional-write/` against the loaded `http-probe` manifest and Cedar grammar |
...
| Tokio process lifecycle | `crates/dekopon-process/src/` | Inline typed-result, Tokio task-panic, cooperative-cancellation, and payload-free tracing tests plus a public doctest; consumed by `dekopon-run`'s `legacy-shell` and `direct-command` nodes and `dekopon-harness`'s cancellable `broker-command` node |
| Shared prompt loop, safe agent-configuration/image-generation meta views, session capability dispatch, mounted skills, opt-in improvement suggestions, and session replay | `crates/dekopon-harness/src/`: the engine and its `PromptError` in `session.rs`, request-one metadata in `bootstrap.rs`, the broker leg and freshness digests in `runtime.rs`, retained history/context in `history.rs` and `context.rs`, gateway-owned conversations in `conversation.rs`, snapshots and their bounded store in `checkpoint.rs`, token accounting in `accounting.rs`, model/effort controls in `control.rs` and `control/transition.rs`, cosmetic dispatch activity in `activity.rs`, prompt tools in `tools.rs`, credential-free views in `meta.rs`, the skills listing and `read_skill` tool in `skills.rs`, the `suggest_improvement` tool and its bounds in `improvement.rs`, recorded-session reconstruction and `replay` in `replay.rs` and `replay/context.rs` | Inline prompt/meta-tool, one-attempt byte-free image output, bounded redaction-shape, composite-dispatch, and stub-broker-socket leg tests, plus inline listing/argument, suggestion-validation, and reconstruction/divergence tests in those modules; module test files in `checkpoint/tests.rs`, `accounting/tests.rs`, `control/tests.rs`, `activity/tests.rs`, and `replay/portable_tests.rs`; recorded-asset coverage in `crates/dekopon-harness/tests/replay_assets.rs` |
...
| Chat gateway configuration, text/image transports, routing, bounded agent sessions, credential-free self-inspection, conversation history, and prompt cache keys | `crates/dekopond/src/` | `crates/dekopond/src/tests.rs` for strict configuration, routing, admission, effective config introspection, conversation replay and eviction, cache-key minting/rotation, generated-image delivery, route-mounted skills read on demand, the per-route `improvementSuggestions` opt-in, and loopback Slack/Discord/Telegram/WhatsApp transports; `crates/dekopond/src/tests/activity_progress.rs` for progress-message ownership, channel pacing, and shutdown drain/abandonment, and `crates/dekopond/src/tests/lifecycle_receipts.rs` for delivery receipts; `crates/dekopond/src/transport/slack/progress/lifecycle_tests.rs` for the activity lifecycle; `crates/dekopond/src/transport/whatsapp.rs` for webhook signature, refusal, saturation, listener, and reply-splitting tests; `crates/dekopond/tests/gateway.rs` for a real `dekopon-brokerd` end to end; `crates/dekopond/tests/examples.rs` for the checked-in walkthrough configuration |
...
package tests. Pin accounting schemas/unknown totals, restore/failure/Stop evidence, authenticated
controls, bootstrap schemas, sender isolation and progress/delivery equivalence. Run privilege
scans, documentation/audit gates and release metadata verification. Wider workspace, packaging and
MSRV gates remain required before publication; a new crate name needs separately authorized
publication bootstrap, not a changed release-list entry alone.
...
- Prompt mode always offers the `bash` model tool, whose `script` argument runs on `dekopon-shell`; `prompt --skill` adds `read_skill` and `prompt --suggestions` adds `suggest_improvement` (the gateway may add its own meta tools; see `crates/dekopon-harness/src/session.rs`). Model tool selection and arguments remain untrusted, and a `bash` call carrying no string `script` ends the session.
