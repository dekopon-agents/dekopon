...
| Shared prompt loop, the safe agent-configuration meta view, session capability dispatch, descriptor-backed asset integration, mounted skills, and opt-in improvement suggestions | `crates/dekopon-agent/src/`; the typed-output intake and `chat-asset:` reference discovery in `attachment.rs`, the skills listing and `read_skill` tool in `skills.rs`, the `suggest_improvement` tool and its bounds in `improvement.rs` | Inline prompt/meta-tool, explicit-send slot and descriptor-reference, bounded redaction-shape, composite-dispatch, and stub-broker-socket leg tests, plus inline listing/argument and suggestion-validation tests in those two modules |
...
Generic model types and transports belong in `dekopon-model`; the shared bounded prompt/tool loop belongs in `dekopon-agent` (`prompt.rs`), which `dekopond` and external clients embed. `dekopon-model` holds no image client: producing an image is a provider effect, so the credential lives behind the broker and the gateway only delivers the bytes. Keep provider credentials inside the broker. Asset bytes cross broker IPC only as read-only descriptors; typed metadata stays out of the shell payload path. Mock network protocols in tests; never read or import another application's credential store.

Asset proposals retain exact `chat-asset:<N>` references. The agent resolves distinct string leaves
into pinned read-only descriptors and uses the protocol's typed `InvokeAssets` and
`AssetInvocationOutcome`; no provider result byte-object parser or marker expansion remains.
`attachment.rs` owns the gateway-facing registration/send traits; `dekopond::asset` owns scoped
numbering, retention and output sniffing. Update the descriptor, lifetime, queue, transport and
failure-note tests together. See [asset handles](dekopond.md#asset-handles-and-delivery).
...
Pull-request caches are restore-only. The workspace-test lane uses SHA-pinned `Swatinem/rust-cache` for Cargo registry/Git dependencies and compiled dependency artifacts in its own `target/`, alongside the existing read-only sccache compiler cache. Its `workspace-tests-v1` shared key matches the dedicated test-dependencies job in `.github/workflows/cache-warm.yml`; runner, toolchain, Rust environment and manifest/lockfile hashes also qualify the cache. The action's default pruning excludes workspace crates, linked test executables and incremental artifacts; `cache-bin: false` leaves installed tools alone. Other PR lanes retain the existing registry/compiler caches. The warmer saves test dependency artifacts only on `main`, and still writes the other lanes' default-branch registry cache capped at 512 MiB plus granular sccache objects. Warmers compile but execute no tests and are not a second validation gate. The first PR introducing this key is cold until a main-branch warmer populates it; cache restore/save overhead and actual build times, not a lookup hit alone, determine whether it helps.

The required `test (Rust)` job now separates `Compile workspace tests` (`cargo test --no-run`) from `Run workspace tests` (library, binary and integration tests). Both run on the same runner and reuse its linked binaries; the run step still invokes Cargo for freshness checks and may rebuild if inputs change. Default example compilation remains in the compile step. Doctests run exactly once in `Test documentation`, because stable Cargo does not support combining `--doc` with `--no-run`. No test harness or parallelism setting changes.

Linux releases also restore main-owned dependency caches. The `release-dependencies` matrix in `.github/workflows/cache-warm.yml` runs the exact release CLI build on native `ubuntu-24.04` (amd64) and `ubuntu-24.04-arm` (ARM64): `cargo build --release --locked -p dekopon-brokerd -p dekopond --target <TARGET>`. It installs the same pinned toolchain/target and uses the same SHA-pinned Swatinem action and `linux-release-v1-<TARGET>` shared key as `.github/workflows/release.yml`. Neither side uses a compiler wrapper; Swatinem sets `CARGO_INCREMENTAL=0` before constructing the key. The action additionally keys by host OS/architecture, Rust toolchains/environment and dependency manifests/lockfile, with a dependency-hash fallback; a cold or evicted cache remains a normal build. This is separate from the PR test cache and the stable lane's release-profile `cargo check` compiler objects.

The release warmer runs only on `main`, on the workflow's matching pushes (including release workflow changes) or a manual dispatch against `main`. It builds binaries only to populate dependency artifacts: no tests, provider downloads, archives, attestations or publication. Only successful main builds may save. Workspace crates/final executables are pruned from the saved cache, so releases still build the tagged project and perform final linking. Linux tag jobs restore only and create no new tag-specific copies. macOS retains its existing tag-local retry cache; package/publication caches are unchanged.

Warmers and tags are independent: if a tag arrives before its warmer completes, that release may build cold or restore an older compatible dependency cache. Wait for **Cargo cache warm** on the intended main commit when a warm release matters; the workflow does not publish or gate a release. Cache archives are immutable and new dependency/toolchain generations may coexist until GitHub evicts them. Stable target keys avoid one new copy per tag, but do not impose a repository-wide byte cap. Inspect compressed cache sizes and restore/save overhead after the first fill; GitHub's cache quota/eviction still applies. Existing historical tag caches are not deleted by this change.

The tag-triggered release performs only the release-specific tag/version, changelog, and publication-plan checks before building and attesting three platform archives, creating the GitHub release, and publishing every public crate in dependency order. The authorized tag push is the single publication gate: the `crates-io` environment remains part of the short-lived trusted-publisher OIDC identity but has no required-reviewer rule. A manual dispatch against an existing tag is only recovery; it packages and publishes crates while skipping platform builds, the existing GitHub release, and immutable crate versions already present. Every public crate needs a crates.io GitHub trusted-publisher entry for `dekopon-agents/dekopon`, `release.yml`, and that environment; bootstrap a brand-new crate name only under explicit authorization, then register it and revoke the bootstrap credential. Published versions and tags remain immutable. The complete operator checklist lives in the root [`README.md`](../README.md#maintainer-release-process).
...
# test (Rust): fixtures, compilation, execution using the same target/, then doctests once.
...
The script builds `dekopon-brokerd` and `dekopond`, starts one pinned OpenObserve container with an isolated Docker volume, and drives a private local-transport turn using a Python standard-library model stub whose tool call runs the in-tree `cli-probe` provider's `probe upper` under a real authorization. It requires `transport.receive`, `gateway.message`, `gateway.session`, `broker.invocation`, `provider.compile`, and `provider.invoke`, with invocation trace continuity from the transport's receipt onward (startup compilation may use a separate trace). A smoke-only shipper ingests actual daemon JSON stdout; complete bounded queries and ingestion counts independently prove native trace/span correlation for both daemons. Local, shipped, and remote records must carry the payload sentinel and exclude the fake credential sentinels. Production logs remain stdout-only. The script unconditionally cleans up owned processes, container, volume, and temporary configuration. Run `python3 examples/otel-traces/test-smoke.py` for the correlation, retrieval, ingestion, and redaction failure controls.
...
provider_wit=$(mktemp -d)
cp crates/dekopon-provider-sdk/wit/provider.wit "$provider_wit/provider.wit"
wkg build \
  --wit-dir "$provider_wit" \
