...
## Unreleased core session controls

`select_model` and `set_effort` are proposals, not provider capabilities. Fresh broker-authenticated
`authorizeControl` evaluates `agent.prompt` and each changed-dimension Cedar action, subject to
...
harness. See [harness.md](harness.md) for safe-yield fencing and current recording/validation gaps.

## Conversation memory as a trust surface

**Status: current.** A route set to `mode: persistent` implements the [Conversations](dekopond.md#conversations) contract: a per-sender history, bounded by a sliding window, an idle timeout, and a process-wide ceiling, held in the gateway process's memory through `dekopon-harness` and replayed into the next prompt. `oneShot` is the default, so a route that does not ask for memory still runs each message as an independent session that starts from an empty prompt.
...
Invalidation now compares full scoped descriptions/schemas/command words plus the broker's random
startup epoch, not identifiers alone. Broker configuration is startup-fixed, so restart changes
that epoch even for A→B→A. Invalidation cannot erase content already shown in chat or exported
under payload consent. Ordinary live-loop boundaries revalidate the authenticated surface and epoch; changed or uncertain
...
- **Bounded portable evidence.** Harness history retains whole tool groups and actual execution observations separately from bounded model context, with excerpts/digests/truncation and incomplete-work warnings. This retains more untrusted provider text than the former question/final-answer window; it does not detect injection or prove effects from model narrative.
...
surfaces are live limitations. Immediate in-memory invalidation compares full scoped metadata and broker startup epoch. Durable `authority-bound` continuity instead commits effective capability metadata,
