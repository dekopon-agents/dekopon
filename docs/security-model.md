...
- **What is untrusted.** Message text end to end, bounded to 16 KiB before it reaches a model and 8 KiB on the way back out. The names and media types of files attached to a chat message are the same untrusted text: the gateway reports what arrived so a model does not deny a screenshot that exists, and the 16 KiB bound covers the message and that note together. It never fetches a file's contents, so no sender-supplied bytes beyond the message itself reach a model. The agent's `instructions` from the catalog are untrusted model text by the same definition: they shape how an agent answers and can never assert identity, name a principal, or widen a capability. Broker policy never reads that field.
