...
- broker-generated authorization receipts and audit records;
...
Attested proposals are the one sanctioned indirection, and they are narrow. A peer whose owner-configured identity carries an `attestor` grant may attach a typed `Attestation` naming a canonical external subject (`slack.t0123abc.u9xyz`, `discord.123456789012345678`, `whatsapp.16034700182`, `tel.16034700182`) alongside its proposal. Every one of them carries a name a real service verified before the message reached a transport; there is no service here for an identity nothing authenticated. The subject is transport routing metadata, not a principal and not authority: the broker — never the peer — resolves it through owner-controlled `principals`, and an unmapped subject resolves to nothing rather than minting a principal on demand. The grant's optional `namespaces` bound which subjects a peer may speak for, matched on segment boundaries so `slack.t0123abc` cannot reach workspace `t0123abcx`; without them it speaks for exactly the mapped subjects. A refused attestation is an audited denial under the peer's own principal with a stable reason (`attestation-denied`, `unmapped-subject`), never a silent error, so a compromised or misconfigured gateway leaves a decision trail. The resulting context is bound to a `via` naming the attestor and an `agent`, and policy sees both as required `context.via` and `context.agent`. A direct peer carries neither, so no policy can grant it a capability; every capability runs inside an attested agent session. Driving an agent at all is a separate `agent.prompt` statement, so a mapped subject with no such grant is refused (`agent-denied`) before any capability is considered.

A chat scope also states what started the turn: `trigger` is `message` for a person's message, `wake` for a session a scheduled wake started, and `probe` for a watch's unattended check. Policy sees it as `context.trigger`. A wake reuses the subject and conversation of the authenticated message that scheduled it, so it can never speak as anyone else or anywhere else, and every wake session is authorized afresh. A probe leg is refused every capability whose effect is not read-only (`probe-write`) before policy is consulted: an unattended loop may look, never act.
...
store. Resolution occurs once after the decision audit record is emitted and is pinned to that
...
OpenAI's fixed ChatGPT/Codex subscription endpoints, or OpenRouter at the fixed `openrouter.ai`
host using its API key. Model bearer tokens are read from named
...
OTLP trace and log fields carry prompts, model responses, command arguments and output, and component input; there is no payload gate and no metadata-only mode ([goal 2](design.md#constitution)). Bearer tokens are omitted, and so is raw internal error text. A provider's own typed failure is the one untrusted failure string that is carried: `error.code` and `error.message` on the `broker.execution` record and the `broker.execute` span, bounded at 128 and 1024 bytes, because without them the reason an invocation failed exists nowhere in the trace ([goal 2](design.md#constitution)). They are read as provider-authored text, never as a broker statement. OTLP lifecycle logs are operational data rather than authorized invocation evidence. Final text and machine-readable outputs remain untrusted data. Terminal table cells in auth status remove control characters. [`observability.md`](observability.md) is the complete telemetry contract.
...
- **Attachment does not authorize delivery.** Successful typed outputs join the gateway's bounded
  ...
  per turn. Sent state lasts for the generation; failed delivery neither retries nor clears it.
  Failed/cancelled turns send nothing. Chat/model credentials remain gateway-side and provider
  credentials broker-side. Broker output files are unlinked read-only handles, not a shared path
  mount; closing the final descriptor reclaims disk. Bytes never enter proposal/result JSON or asset
  audit fields. See [adapter format support and lifecycle](dekopond.md#asset-handles-and-delivery).
- **Liveness is presentation, not authority.** It is opt-in, uses the same chat bot credential already terminating in this process, targets only service-native channel/thread/message coordinates derived from the authenticated envelope, and may be overlaid per conversation kind — a budget knob, never a change to what a transport can render. The model supplies no coordinate, progress text, emoji, cadence, or fallback: every progress line is assembled from operator templates and the session's own counters, and the only model-authored text a surface can carry is the streamed answer itself. Discord typing and Telegram chat actions expire; Slack Agent status is explicitly returned to active, while its classic/free fallback is the one fixed `:tangerine:` reaction and removes only a reaction that generation successfully added. Liveness failures carry only low-cardinality categories and never affect the answer. An authenticated stop — a native Stop, a cancel button, a configured stop word, an operator shutdown, or a route's wall-clock bound — is transport control rather than model text, and only the subject who started a session may stop it: it cooperatively prevents later model/tool/broker work and suppresses history, answer delivery, attachment delivery, and durable recording, but cannot roll back a model request or provider effect already in progress.
...
The WhatsApp transport adds one public wakeup surface to the unprivileged daemon. It terminates no TLS and exposes no admin method. GET proves only knowledge of the separate verification token. POST identity is accepted only after one exact HMAC-SHA256 over the untouched bounded bytes; the signed `messages[].from`, never profile/display text, becomes `whatsapp.<wa_id>`, while the exact configured WABA and receiving-phone tuple stays transport-derived scope. Message-ID replay handling is bounded process memory, so restart can admit a redelivery and a post-200 crash can lose queued work; no durable exactly-once claim is made ([`design.md`](design.md#non-goals)). WhatsApp ignores webhook URLs, resolves media IDs under its configured phone, and bounds PNG/JPEG downloads and PNG/JPEG uploads to 5,000,000 bytes; metadata and byte signatures are validated without full image decoding. Captionless images use the same scoped attachment inventory. Other media hosts, redirects, proxies, userinfo, fragments, and unexpected ports are refused; this conservative host pin may reject future Meta CDN changes. Outbound Graph sends are never blindly retried because a timeout after transmission has unknown outcome.
...
The scope stops at `(agent, configured transport, transport-derived conversation identity)`. It is not global agent memory, team memory, a durable shared namespace, or automatic replay across channels, threads, transports, routes, or agents. `recall: journal` does bring the same key's window back across restarts and idle or capacity eviction, but never across a grant change. Every participant starts a fresh attested leg and must independently be authorized for the agent on every message. If their capability identifier sets differ, the conservative grant comparison drops the shared window rather than carrying content across those grants.
...
- **In the gateway.** History lives in the gateway process and is never sent to the broker. Without `sessions.journal` it is never written to disk and dies with the process. With it, journal routes also keep their window in operator-owned `0600` files on the gateway's volume, and a person's text outlives the process for up to `forgetAfterMs`; the operator is inside the trust boundary ([design.md](design.md#non-goals)). `recall: platform` adds no storage: it rereads what the chat service already shows every participant. History also reaches the operator's telemetry store, for as long as that store retains it.
- **Grant-set invalidation.** Described above: the granted capability set travels with the conversation, and a change drops it.

### Where the text goes

Not into the broker. `dekopon-brokerd` holds provider credentials and emits one metadata-only audit record per decision, in which a provider's output survives only as a digest, and its records exclude inputs, outputs, paths, queries, headers, and bodies. Putting conversation text in that process would place the most sensitive content in the system inside the most privileged one, next to a record built specifically not to contain it, and it would turn a log of what was *authorized* into a store of what was *said*.
...
invocation-bound transport and conversation claim that must be canonical for the sender's service.
The canonical conversation enters Cedar context, as the
...
([`upgrading.md`](upgrading.md)). What is reserved is what the owner declared: a
...
The statically linked native client enforces exact authority/port and method grants, request count and byte bounds, HTTPS by default, explicitly authorized plaintext to loopback destinations and to the exact hostnames the broker owner listed in `http.plaintextHosts`, DNS address validation and pinning, sensitive-header ownership, no redirects, no ambient proxy, no automatic decompression, and bounded response collection. The independent `http.nonPublicHttps` list permits only exact DNS authorities and ports whose every resolved address is private unicast; `http.extraCABundles` adds broker-owned PEM roots alongside public roots for all HTTPS destinations without granting egress. This does not apply to gateway/model clients, OTLP or the OCI provider manager. Its evidence contains method, authorized authority, status, and byte counts—not paths, queries, headers, or bodies. Only an owner-authored HTTP constraint set with `propagateTrace: true` permits the broker to send its egress span's `traceparent`; operators opt in only first-party destinations inside their trust boundary. Provider-supplied `traceparent` and `tracestate` headers are refused on every grant.
...
`dekopon-broker` wraps that host with a transport-independent trusted context, deny-by-default Cedar authorization, owner-authored execution constraint sets validated at startup against provider metadata and host ceilings, single-use authorization construction, stable public outcomes, digest evidence, and metadata-only audit: every decision is a `broker.decision` or `broker.execution` log record inside the trace, emitted before any `AuditLog` sink sees it. Human/service actor principals must match transport principals; an agent actor's identity reaches policy as `context.agent`.
...
Decisions are explainable without being leaky. Every audit record carries `policy.ids`, the identifiers of the policies that determined the outcome (an `@id("…")` annotation names them stably), and `policy.digest`, a fingerprint of the policy set and world evaluated. Policy source itself reaches an operator only through startup errors — never through a per-request decision, an audit field, or a `Debug` rendering. Inputs, provider outputs, URL paths/queries, headers, bodies, and credentials are absent from audit records; a failed record's bounded `error.code`/`error.message` are the provider's own account of its refusal, not its output, which stays a digest. Authorization decisions are recorded before execution. When provider work may already have completed with no terminal record — terminal evidence failed to hash, or an embedding's bounded `InMemoryAuditLog` refused the outcome — the error says so. `BrokerError::unaudited_outcome` makes that distinction structural rather than a matter of error text, and `dekopon-brokerd` preserves it across the wire as the `outcome-unaudited` failure code so a client can tell "nothing executed" from "the effect may have happened".

`dekopon-brokerd` runs `TraceOnlyAuditLog`, which keeps nothing: the log record is the audit, on stdout always and as an OTLP log record when `telemetry` is configured, so losing the exporter loses audit. There is no tamper-detection, rollback protection, or crash-recovery guarantee, and [`design.md`](design.md#non-goals) rules out building one. The bounded `InMemoryAuditLog` is for tests and embedding.
