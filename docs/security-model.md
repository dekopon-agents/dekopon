...
Attestor namespaces and `via` policy now constrain a distinct gateway peer, rather
than a broker-UID process. A compromised gateway can still speak for subjects inside
its configured attestor grant; this is not independent per-process attestation or
general tenant isolation. Each UID remains its own trust domain. Broker-UID compromise
and host/root compromise are outside this boundary. Owner-only local broker clients
remain supported with a private parent and **0600** socket; the gateway's local chat
socket stays **0600**, owner-only and development-only.
...
store. Resolution occurs once after the decision audit record is appended and is pinned to that invocation; there
...
The isolated `dekopond auth` namespace manages model-account login against fixed authentication
hosts, before gateway configuration, transports, or runtime creation. Within that namespace, `dekopond auth chatgpt export` is the single command in Dekopon that writes credential material in the clear; it is documented, gated behind a required `--expose-credential`, refused when standard output is a terminal unless `--allow-terminal` is passed, and warns on standard error that the exported copy is invalidated by the next refresh. Its output form is chosen explicitly, it fails rather than emitting a partial document, and it makes no network request. See [`chatgpt-credential.md`](chatgpt-credential.md). Auth performs no model inference, provider credential resolution, authorization decisions, or external effects. Provider readiness in local config is descriptive data, not a verified connection.
...
ChatGPT authentication and inference hosts remain fixed to `auth.openai.com` and
...
script. Skills are untrusted model text and grant no authority. Gateway suggestion tools are
explicit opt-ins to bounded model-authored telemetry, as described below.

By default, OTLP trace/log fields omit prompts, model responses, and component input/output; the payload opt-in adds them to every sink the process writes. Bearer tokens and raw untrusted errors are omitted in both modes. The operator-selected telemetry endpoint still learns execution metadata such as service/model/provider/capability identifiers, timings, outcomes, and source locations. OTLP lifecycle logs are operational audit data, not authorized invocation evidence or a substitute for the broker's append-only JSONL log. Final text and machine-readable outputs remain untrusted data. Terminal table cells in auth status continue to remove control characters.
...
- **Provider attachments are courier work, not authority.** Producing an image is a provider effect like any other external write: the broker authorizes it, audits it, and holds the credential, and this daemon holds no image credential at all. What the daemon adds is delivery, and only on a route whose owner wrote `providerAttachments: { maxPerReply: N }`. A successful capability result's reserved top-level `attachments: [{mediaType, base64}]` is stripped by the session's broker leg before the result reaches the shell, and each entry is accepted only as `image/png`, decodable base64, a real PNG signature, at most 8 MiB decoded, and within the route's per-reply ceiling counted across the whole session. Accepted bytes leave the session through a request-local slot and go only to the authenticated envelope's reply target; the model reads `attached: [{mediaType, bytes}]`. Bytes never enter model transcripts, telemetry payloads, in-process or durable conversation memory, provider JSON, broker protocol, evidence, or audit, and the slot is dropped unread when a session fails or is cancelled. A route without the opt-in still strips the key, so bytes cannot reach the shell merely because nowhere can deliver them. Attachment content is untrusted provider output and no content-safety claim is added beyond the producing service's behavior. Every refusal is one fixed gateway sentence the model reads plus one audit record, never a failed invocation — the effect already happened and must not be retried.
- **A chat attachment reaching a capability is new reach, configured per route.** `chatAssetInputs` lists the capability identifiers whose input may name one of the conversation's own attachments as the exact string `chat-asset:<N>`; the session's broker leg replaces each marker with `data:<mime>;base64,<bytes>` before the proposal is submitted, and an unlisted capability receives the string verbatim. An unknown identifier in the list is a startup failure. The bytes come from the same metadata-only store and the same transport credential the model-facing attachment tool uses, so nothing new is read and nothing is granted; what is new is that a sender's file can now land inside an authorized provider's input. It is bounded by arithmetic: image media types only, at most three expansions per invocation, at most 8.5 MiB decoded per invocation, at most twelve expansions per session, and 8 MiB per attachment, all separate from the model's four `fetch_chat_asset` calls so neither can exhaust the other. The session bound is the one that matters most here, because expansion happens *before* the broker decides: without it a script could spend its whole capability budget proposing a listed capability and pull attachments off the chat service on every proposal, and a policy that denied all of them would not have prevented a single download. A refusal submits no proposal, is reported to the model as one fixed sentence, and is audited with a stable reason. The marker is matched exactly, so text a person actually typed is never rewritten, and a capability still validates its own input.
- **In-flight activity is presentation, not authority.** It is opt-in, uses the same chat bot credential already terminating in this process, and targets only service-native channel/thread/message coordinates derived from the authenticated envelope. The model supplies no coordinate, status text, emoji, cadence, or fallback. Discord typing and Telegram chat actions expire; Slack Agent status is explicitly returned to active, while its classic/free fallback is the one fixed `:tangerine:` reaction and removes only a reaction that generation successfully added. Activity failures carry only low-cardinality categories and never affect the answer. Slack Agent Stop is authenticated transport control rather than model text: it cooperatively prevents later model/tool/broker work and suppresses history, answer delivery, attachment delivery, and durable recording, but cannot roll back a model request or a provider effect already in progress.
...
Not into the broker. `dekopon-brokerd` holds provider credentials and a metadata-only append-only JSONL audit log in which a provider's output survives only as a digest, and its records deliberately exclude inputs, outputs, paths, queries, headers, and bodies. Putting conversation text in that process would place the most sensitive content in the system inside the most privileged one, next to a record built specifically not to contain it, and it would turn a log of what was *authorized* into a store of what was *said*. The gateway already reads the message and writes the answer, so keeping the history there adds no new reader.
...
And not into the [prompt cache key](dekopond.md#the-prompt-cache-key). Every model request declares one so the provider's prefix cache has somewhere to route the requests that share a prefix, and it is minted from entropy rather than derived from the private subject or shared conversation identifier. A canonical subject can be a phone number, and a hash of one is a stable pseudonym; either would tell a model provider that two conversations months apart belong to one person, which is the linkage the metadata-only telemetry default exists to withhold and a worse thing to hand a third party than to hand your own sink. The minted key rotates whenever the private or shared conversation generation it names is evicted and whenever the process restarts, so it never accumulates into a durable identifier for anybody or any service-native conversation, and it is a routing hint that confers nothing: a request carrying a key is authorized exactly as one without it, which is to say by the broker, per message.
...
configured transport and agent keys still prevent aliasing another route's history.
...
`dekopon-broker` wraps that host with a transport-independent trusted context, deny-by-default Cedar authorization, owner-authored execution constraint sets validated at startup against provider metadata and host ceilings, a bounded replay ledger, single-use authorization construction, stable public outcomes, digest evidence, and metadata-only bounded in-memory logs or owner-only append-only JSONL audit. Human/service actor principals must match transport principals; an agent actor's identity reaches policy as `context.agent`.
...
The committed first privileged-provider design is documented in [`dekopon-brokerd` contract](../crates/dekopon-brokerd/README.md#boundaries). It preserves the separate broker boundary, treats HTTP imports as structural requirements rather than authority.
