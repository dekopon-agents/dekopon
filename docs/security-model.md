...
Decisions are explainable without being leaky. Every audit record carries `policy.ids`, the identifiers of the policies that determined the outcome (an `@id("…")` annotation names them stably), and `policy.digest`, a fingerprint of the policy set and world evaluated. Policy source itself reaches an operator only through startup errors — never through a per-request decision, an audit field, or a `Debug` rendering. Inputs, provider outputs, URL paths/queries, headers, bodies, and credentials are absent from audit records. Authorization decisions are recorded before execution. When provider work may already have completed with no terminal record — terminal evidence failed to hash, or an embedding's bounded `InMemoryAuditLog` refused the outcome — the error says so. `BrokerError::unaudited_outcome` makes that distinction structural rather than a matter of error text, and `dekopon-brokerd` preserves it across the wire as the `outcome-unaudited` failure code so a client can tell "nothing executed" from "the effect may have happened".
