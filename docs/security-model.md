...
- **What terminates in the daemon.** Chat bot credentials (Slack app-level and bot tokens, Discord and Telegram bot tokens) and model credentials (an OpenAI-compatible API key, Dekopon's own ChatGPT device-flow credential file, or an explicitly named OpenAI Images key). All of them are named in configuration as *environment variable names*, never values, and load into `Redacted` wrappers; a missing image-generator or chat credential is a startup failure naming the variable and never its value. A name that is not a valid variable name is rejected at startup, so a token pasted into that field is a refusal rather than a plaintext secret in a config file.
