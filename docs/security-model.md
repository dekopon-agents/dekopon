...
"In memory only" is a durability property and not an isolation one. In the current single-UID deployment any process running as the owner can read the gateway's memory, exactly as it can already act as the configured gateway peer. Operating-system paging and core dumps are outside what the daemon controls. The development transport authenticates that local UID rather than the declared person; it trusts the caller to supply a canonical subject, while configured-transport and agent components prevent the claim from aliasing another transport's or agent's state key. None of this is new authority; all of it is the single-UID caveat applying to a new kind of content, and a dedicated gateway UID is what turns it into a real boundary.
