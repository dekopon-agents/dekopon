...
3. Add one broker-mediated read-only integration with narrowly scoped host I/O, authenticated identity, and evidence before introducing external writes.
...
5. Evolve the immediate Wasmtime host into a broker-owned asynchronous host with a shared engine, compiled component cache, fresh bounded stores, and Tokio-integrated host calls.
