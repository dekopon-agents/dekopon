...
The repository-level black-box observability test lives with its runnable example at [`examples/otel-traces/smoke-test.sh`](../examples/otel-traces/smoke-test.sh). It builds `dekopon-brokerd` and `dekopond`, starts one OpenObserve container, drives one local-transport gateway turn through both daemons against a stub model and the authorized echo provider, and searches both streams: the trace stream for the required spans and no sentinel provider input, and the log stream for a lifecycle record carrying the same `trace_id`. It removes the container and its volume afterward ([details](../docs/development.md#openobserve-otlp-end-to-end-test)).
