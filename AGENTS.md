...
- Construct expensive resources once: HTTP/model clients, Wasmtime engines, linkers, compiled components, and worker threads live at process or session scope, never per request/message/invocation. CI pins the Wasmtime shape: one `Engine::new` in production code and one `host::engine(` caller outside the SDK, so one engine, one linker and one `InstancePre` per provider are shared by every call while each invocation gets a fresh store, and `dekopond`'s normal dependency tree contains no wasmtime at all.
