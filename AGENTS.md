...
| The session engine: bootstrap, history and context bounds, the execution journal, accounting, model/effort controls, or dispatch activity | [`docs/harness.md`](docs/harness.md): the unprivileged runtime's contract, its bounds, and what the journal is not |
...
- Never measure a bound by serializing. `serde_json::to_vec(x).len()` inside a ceiling check, a `while` condition, or an eviction loop makes every write O(corpus) under the lock. Keep a running byte count where the data changes and serialize once, at the save.
- A check that runs per invocation must be local and O(1); a broker round trip belongs at the turn boundary. Before adding a freshness, admission, or catalog fetch, count how many a single model turn with N tool calls already makes.
- Do not build a distributed-store contract for one process. Leases, compare-and-swap revisions, fencing tokens and format versions are the price of a second reader; dekopon runs as one executable on one machine, so a bound belongs on the field it bounds and HA belongs in front of the process. A ceiling that exists only to size a heap reservation must never become a config-validation failure an operator has to edit around.
- A backoff that parks a shared resource (a channel, an installation) must make later callers wait, not fail. Only the caller that exhausted its own budget is refused, and the refusal says how long the backoff has left; a penalty must never turn unrelated sessions' answers into instant `post-capacity`.
- Every code path that creates the same externally visible thing goes through the same pacing and ownership gate. `chat.postMessage` and `files.completeUploadExternal` both create a channel message; grep for the platform endpoints, not for the helper.
- A `reason` assigned inside a loop is never downgraded by a later iteration: keep the most severe (`policy-error` over `policy-denied`) and pin it with a test that changes two dimensions at once.
- `Err(error) => { tracing::error!(cause_type = "…") }` without `cause = %error` or a typed kind is a discarded cause even though it logs; `map_err_ignore` does not catch it. A category token (`model-control`) is not a cause.
- Tightening validation on an existing config field (a new grammar for `models[].name`) is a breaking change: the refusal names the field and the offending value, and `docs/upgrading.md` gets the entry in the same PR.
- A reader enforces the writer's ceiling. When a producer caps a collection (`MAX_TOOL_CALLS_PER_TURN`), the reconstruction that decodes it rejects anything larger before iterating; an unbounded inner loop over a peer-supplied array is a hang.
- Prompt text is a mirror too. When a mechanism changes (bootstrap replaces discovery), grep the tool descriptions for the sentence that told the model to do it the old way, and pin the new wording.
- A flaky-test repair keeps the assertion that made the test worth having. If the racy step is removed, the property it proved gets a new home, or the PR says the coverage is gone; "without weaker assertions" is a claim a reviewer will diff.
...
- `target/` grows past 25 GB with test binaries, and a full disk surfaces as LLVM or linker "IO failure" errors rather than test failures. Recover by deleting `target/debug/incremental` and the large test executables under `target/debug/deps`; they regenerate. `CARGO_INCREMENTAL=0` keeps it from regrowing. Every worktree owns its own `target/` (a workspace test build is ~14 GB); prefer crate-scoped commands while iterating, and `cargo clean` a worktree before handing it back or leaving it idle — the shared compiler cache makes the rebuild cheap, the leftover binaries are never reused.
- Tests that capture tracing events install the process-global subscriber with a thread-ID filter (`dekopon_test_support::CaptureLayer`, as `crates/dekopon-harness/src/accounting/tests.rs` does), never a scoped `with_subscriber`. Callsite interest is cached per process: a scoped dispatcher races a sibling test's first registration of the same callsite, the event is cached `never`, and the test fails only under parallel load with `missing <event>`.
