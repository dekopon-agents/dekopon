...
- **Provider fixtures:** `examples/providers/{cli-probe,clock-probe,http-probe,memory-reservation-probe,storage-probe}` are separate Cargo workspaces with their own `Cargo.lock`; root workspace commands do not cover them. Their checked `*-provider.wasm` files are generated; JSONPlaceholder and memory-chat are gitignored fixtures fetched by script.
...
| Wasm components, prompt tools, model endpoints, or limits | [`docs/development.md`](docs/development.md), [`crates/dekopon-agent/README.md`](crates/dekopon-agent/README.md), and [`crates/dekopon-shell/README.md`](crates/dekopon-shell/README.md): surviving host and orchestration contracts |
| Daemon traces, OTLP logs, telemetry redaction, audit event names, or OpenObserve | [`docs/observability.md`](docs/observability.md): signal contents, configuration, and the CI-gated audit-event list |
| Skills, `read_skill`, `suggest_improvement`, or evaluating a changed instruction before it ships | [`docs/improvement.md`](docs/improvement.md): the operator-driven improvement loop and what is absent |
...
| Broker-mediated provider HTTP, host imports, or broker client mode | [`crates/dekopon-brokerd/README.md`](crates/dekopon-brokerd/README.md#boundaries): the process boundary, buffered HTTP contract, authorization inputs, and staged delivery |
...
- One trace, complete ([goal 2](docs/design.md#constitution)): a run's prompts, scripts, command words with arguments, decisions, and egress ride the message's W3C trace; only secret bytes and the daemons' own credentials stay out. Bound an attribute's size, never the span count.
- The non-goals in [`docs/design.md#non-goals`](docs/design.md#non-goals) reject code kept on idempotency, durability, endpoint-reflection, or end-user-privacy grounds.
...
- `dekopond` and `dekopon-brokerd` remain separate processes. External-write authority exists, so this is a live invariant: the gateway must never gain policy, provider credentials, or an authorization path of its own. CI rejects `dekopon-broker`, `dekopon-broker-host`, `dekopon-brokerd`, `dekopon-http-host`, `dekopon-storage-host`, or `dekopon-policy` in the normal dependency tree of `dekopond`; the unprivileged `dekopon-broker-protocol` client is the one broker-named crate the gateway may carry. Conversely, CI rejects `dekopon-agent`, `dekopon-shell`, `dekopon-process`, or `dekopon-config` in the normal dependency tree of `dekopon-brokerd`; `dekopon-model` is absent from that list because the broker's `kind: chatgptSubscription` credential resolves through `dekopon_model::chatgpt::CredentialFile`, the shared refresh implementation. *Committed direction:* `credential`/`credentialByAgent` bindings will be replaced by public DRNs, preserving that refresh behavior ([migration requirements](docs/design.md#legacy-credential-bindings)).
...
- Emit every refusal or failure cause once.
...
- Format, lint, test, and `wasm32-unknown-unknown` check of every `examples/providers/*/Cargo.toml` ([Provider example workspaces](docs/development.md#provider-example-workspaces)), plus wasm32 checks of `dekopon-provider-sdk`, `dekopon-provider-clock`, `dekopon-provider-http`, and `dekopon-provider-storage` with each storage feature on its own.
...
- `test (Rust)`: `cargo test --workspace --all-features --locked`, then the same with `--doc`, on the `rust-toolchain.toml` compiler, which is also the MSRV. `quality (stable)` additionally requires `broker-host components (Linux ARM64)`, which runs only `cargo test -p dekopon-broker-host --test host --all-features --locked` on `ubuntu-24.04-arm`.
...
- `CLI smoke tests`: builds the two daemon binaries, runs their help and example commands, and path-installs each with `cargo install --debug --locked --path`.
- Separate jobs cover the OpenObserve OTLP smoke test and the Helm chart. `actionlint .github/workflows/*.yml` and `shellcheck <SCRIPT>` are the local checks for workflow and script edits. The branch-protection contexts are `quality (stable)`, `test (Rust)`, `package crates`, `CLI smoke tests`, and `dependency policy`: renaming one of those jobs leaves a permanently pending required check.

## Toolchain facts

- `rust-toolchain.toml` pins one exact Rust, with clippy and rustfmt, for the workspace, the provider fixtures, and every CI job (each installs it with a bare `rustup toolchain install`); `ci/toolchain.env` pins wasm-tools and wkg. Both carry the lockstep comment naming everything that moves with them: `Cargo.toml` `rust-version`, Wasmtime and wit-bindgen in the root and five fixture manifests, `deny.toml`'s skips, and a fixture rebuild. Nothing else repeats those versions.
...
- Rebuilding a repository-owned provider `.wasm` needs exactly the `rust-toolchain.toml` compiler and the `ci/toolchain.env` wasm-tools through the fixture's `build.sh`; `examples/providers/build-component.sh` reads both files and refuses any other version, and the `WIT package` workflow (`.github/workflows/wit-package.yml`), not `ci.yml`, rebuilds every checked component and byte-compares it on pull requests and pushes to `main` that touch crates, provider, toolchain, or WIT paths. A deterministic rebuild from unchanged source leaves the artifact unchanged.

## Environment gotchas

- Run `ci/fetch-external-provider-components.sh examples/providers` before `cargo test --workspace`; it installs the checksum-pinned JSONPlaceholder and memory-chat fixtures (gitignored, never committed), without which many tests fail with `NotFound` on `examples/providers/*-provider.wasm`. The CLI smoke and OTLP lanes fetch nothing; they run the in-tree `cli-probe`.
...
- Two tests assume a non-root user and fail under root (containers): the `dekopon-brokerd` unit test `a_secret_file_that_cannot_be_opened_still_names_its_errno` chmods a file to `0o000` and expects an open errno, and the `dekopon-model` unit test `a_rotated_credential_completes_the_turn_when_the_write_fails` makes a directory `0o500` and expects a write to fail. Neither is a regression signal there.
...
- **Config field or CLI flag:** the owning document (`docs/dekopond.md`, `docs/catalog.md`, `docs/cli.md`), the crate README, `CHANGELOG.md`, the example under `examples/`, plus parser tests and black-box CLI tests; JSON/YAML output shapes and exit codes are compatibility surfaces.
...
- **WIT change:** every mirror listed in [Provider contract or host](docs/development.md#provider-contract-or-host), the package version bump, fixtures rebuilt with the pinned toolchain, all four `wkg.lock` files unchanged, and the [Published WIT packages](docs/development.md#published-wit-packages) checks.
- **Dependency:** the root `Cargo.toml` `[workspace.dependencies]` entry, refreshed lockfiles, `cargo deny --all-features check`, `cargo machete`, and the whole lockstep toolchain move when the crate needs a newer Rust than `rust-toolchain.toml`.
