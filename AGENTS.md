...
- The release-profile `cargo check` of all three binaries, and the feature-off check `cargo check -p dekopon-core -p dekopon-capability -p dekopon-protocol --locked`, which is the only gate that compiles those crates without their opt-in `schemars` feature.
- `cargo machete` (CI pins 0.9.2): any unused dependency fails.
- Two `cargo tree` privilege greps over the normal dependency trees of `dekopon-run` and `dekopond`; any hit fails.
...
- `CLI smoke tests`: builds the three binaries, runs their help and example commands, and path-installs each with `cargo install --debug --locked --path`.
...
- Add WASI, host imports, credentials, or authorization arguments to `dekopon-run`'s immediate mode or its broker client subcommands, or a broker crate to `dekopon-run` or `dekopond`.
