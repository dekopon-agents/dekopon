...
- A per-job reservation against a store ceiling (`MAX_STORE_BYTES / MAX_CHECKPOINT_BYTES`) is a concurrency ceiling. Validate every operator concurrency knob against it at config time, name the constant in the refusal, and never let a full store evict everything on its way to an error.
