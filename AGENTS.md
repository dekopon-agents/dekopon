...
| Public DRNs, private secret maps, secret-use bindings, source adapters, or native Basic/Bearer sinks | [`docs/secrets.md`](docs/secrets.md) | It defines the typed proposal, dual authorization, source, projection, path, rotation, and non-disclosure contract. |
| Deployment secrets, 1Password, External Secrets, or delivering a credential file into a cluster | [`docs/1password-eso.md`](docs/1password-eso.md) | It records the deployed secret-store configuration, the bootstrap a human owns, and how materialized files compose with the broker's source adapters. |
