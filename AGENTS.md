...
- `test (Rust 1.89.0)`: `cargo +1.89.0 test --workspace --all-features --locked --no-run`, then the same with `--doc`, after asserting that `rust-version` in `Cargo.toml` equals the job's pin.
