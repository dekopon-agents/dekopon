...
The read is valid inside `invoke` only. `describe` and `run-command` are pure by contract, so a component that reads the clock from any of them traps and that call fails. A command word such as `date` therefore proposes a capability from `run-command` and reads the clock when the broker invokes it. Each read is recorded as a `provider_clock_read` event inside the invocation's `provider.invoke` span.
