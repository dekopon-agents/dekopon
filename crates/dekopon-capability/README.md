...
An `AuthorizedInvocation` carries an `AuthorizationReceipt` — decision ID, authorizing broker principal, policy revision — and `Evidence` entries carry a kind, a digest of canonical bytes, and a media type. Public `InvocationResult` values carry a deserializable `DecisionReference` instead, so a client correlates a decision without receiving authority. A failed result carries the broker's stable `error` classification and, when that classification came from a provider's own typed failure, a bounded `detail` holding the provider's `code` and `message`. *Committed direction:* removed; the trace is the record ([design.md](../../docs/design.md#core-concepts)).
