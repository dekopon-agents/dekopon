...
- `dekopon-run session list` groups the `accounting.model.call` records an OpenObserve stream holds by trace, newest first, and prints `TRACE`, `STARTED`, `TURNS`, `TOKENS`, `OUTCOME`, and `SERVICE` for at most `--limit` (default `50`) sessions, or the same rows with `--json`. Accounting fires in either payload mode, so it lists sessions recorded metadata-only.
- `dekopon-run session show` reconstructs one session's transcript from its exported `agent.model.prompt`, `agent.model.answer`, and `accounting.model.call` records (`--trace-id`) or from a file (`--from-file`; exactly one of the two), and prints it as text or, with `--json`, as the document `replay --from-file` reads back.
...
Prompt and replay retain their accounting finalizer until stdout write/flush. Accepted output,
failed/partial writes and uncertain flushes are distinct from model generation; BrokenPipe retains
its existing CLI exit convention but is not accounted as delivered output.
