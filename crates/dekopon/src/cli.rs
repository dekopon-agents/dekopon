...
pub struct Cli {
    ...
    ///
    /// Optional in the grammar so that a bare `dekopon` produces this crate's own refusal, naming
    /// what was missing, rather than Clap's generic one. It is still required: absent, `run`
    /// returns the documented usage exit code.
    #[command(subcommand)]
