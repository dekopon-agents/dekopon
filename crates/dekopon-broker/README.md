...
Invocation IDs are reserved in a bounded replay ledger before policy evaluation, so repeated denied requests cannot later be reused for execution. Exhaustion fails closed; transport-level quotas must prevent an authenticated peer from consuming the ledger. A broker restart starts an empty process-local ledger; the audit file supplies no replay identities. Authorization is non-cloneable and consumed by provider execution. Public results carry an inert decision ID/broker/policy reference plus digest evidence. Audit records contain identities, routing and policy metadata — including `policy_ids`, the identifiers of the policies that determined the decision, and `policy_digest`, a fingerprint of the evaluated policy set — stable outcomes, timings, output digests, and sanitized HTTP call metadata. They never contain invocation input, provider output, paths, queries, headers, bodies, cookies, authorization values, or credentials.

The in-memory audit implementation is bounded for tests and embedding. `FileAuditLog` opens an
exclusively writer-locked, single-link owner-only JSONL file without following symlinks. It counts
bounded newline-delimited lines in fixed space for the next ordinal, without decoding history or
checking integrity. Unterminated tails are refused without repair. Every append serializes only
`sequence` and `event`, enforces the byte ceiling, then writes and flushes; it does not fsync.
Failed or cancelled appends poison the handle and may leave partial bytes. No rollback, migration,
crash recovery, or total file-size bound is promised. Existing bytes are never overwritten. A constraint set may bind a symbolic `credential:` resolved from a caller-supplied `CredentialStore`; construction fails closed on unknown names, missing HTTP authority, or allowed hosts outside the credential's destination binding.
