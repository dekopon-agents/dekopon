...
The in-memory audit implementation is deterministic and bounded for tests. `FileAuditLog` creates or opens an exclusively writer-locked, single-link owner-only JSONL file without following symlinks, bounds every line and record count, verifies the complete chain before appending, flushes and synchronizes each record, poisons a handle after a failed write, rejects partial final records, and exposes a chain checkpoint, exact-prefix comparison, and verified replay IDs. `dekopon-brokerd` composes it with an atomic separately locked checkpoint file to detect valid-prefix rollback relative to retained local state. Authenticated transport and checkpoint persistence remain service layers; credentials and independent remote/signed anchoring are not implemented.
