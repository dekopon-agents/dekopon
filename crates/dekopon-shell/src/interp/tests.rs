...
fn while_read_walks_every_line_and_then_stops() {
    ...
    assert_eq!(
        output(r#"http-probe.fetch --uri x | jq -r .bodyText | while read line; do echo "[$line]"; done"#),
        "[alpha]\n[beta]\n[alpha]"
    );
...
fn read_binds_several_names_by_splitting_on_whitespace() {
    ...
    // name, as bash does.
    assert_eq!(
        output(r#"echo "alpha beta gamma delta" | { read -r first second rest; echo "1=$first 2=$second rest=$rest"; }"#),
...
fn getopts_reports_a_bad_flag_and_a_missing_argument() {
    assert!(
        output(r#"parse() { getopts "n:" opt; echo "opt=$opt OPTARG=$OPTARG"; }
parse -z"#)
            .contains("opt=? OPTARG=z"),
    );
    assert!(
        output(r#"parse() { getopts "n:" opt; echo "opt=$opt"; }
parse -n"#)
...
fn getopts_is_scoped_to_a_function_because_positionals_are() {
    ...
    assert_eq!(outcome.exit_code, ExitCode::SYNTAX);
    assert!(outcome.output.contains("only valid inside a function"), "{outcome:?}");
}
...
fn a_compound_command_can_be_a_pipeline_stage() {
    ...
    assert_eq!(
        output("echo.echo --a 1 --b 2 | case $(cat | jq -r .a) in 1) echo one ;; *) echo other ;; esac"),
        "one"
    );
}
