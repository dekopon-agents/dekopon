...
impl StdinSource {
    ...
    fn next_line(&mut self) -> Option<String> {
        let lines = self
            .lines
            .get_or_insert_with(|| self.value.as_deref().map(value::to_lines).unwrap_or_default());
...
impl Evaluator<'_> {
    ...
    fn run_getopts(&mut self, arguments: &[String]) -> Result<CommandResult, CommandFailure> {
        ...
        if !optstring.contains(letter) || letter == ':' {
            ...
            self.assign("OPTIND", Value::from(index + 1))?;
            self.write_line(&format!("dekopon-shell: getopts: illegal option -- {letter}"));
    ...
    fn execute_pipeline(
        ...
        let mut input: Option<Rc<Value>> = self.stdin.last().and_then(|source| source.value.clone());
    ...
    fn run_argv(
        ...
        let executed =
            self.dispatch_command(command, arguments, resolution, input, capture_output, from_pipe);
    ...
    fn run_control_word(
        ...
        let executed = match command {
            ...
            "read" => {
                match self.run_read(arguments, input, from_pipe) {
