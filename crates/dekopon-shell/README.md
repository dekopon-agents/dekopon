...
There is no clock. Reading the wall time is ambient authority with no capability to go through — there is no provider for "what time is it", and minting a fake one would be a capability nobody granted and nothing audited — so `date` is not a command here at all. It was an embedder opt-in that no embedder ever set, and a script asking for the time now gets the same "command not found" an ungranted capability gets. The word is free for a provider to claim.
