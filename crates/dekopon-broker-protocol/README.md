...
`dekopon.dev/broker/v1alpha3`; both envelopes are strict-decoded, so a broker and a client from
...
## Core controls (unreleased)

`authorizeControl` carries a strict `ControlProposal`: global invocation ID, opaque
agent/job/session/request/generation scope, one-based sequence, active `surfaceEpoch`, complete
configured-model/effort `from` and `to`, trace and trace-parent. An optional attestation binds to
the same ID. Neither arbitrary policy context nor provider output is accepted.

`ControlClient` pins the host's scope, attestation, epoch and spent attempts. It verifies server
UID and every echoed binding, consumes one pending response, and returns a non-cloneable,
non-deserializable `VerifiedControlDecision`. Only its live exchange constructs that type.
`consume` means admission or `control-denied`, never application, provider execution authority,
or permission to reuse a decision. A dropped/failed exchange fences the client; no automatic retry.
