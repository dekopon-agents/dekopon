...
mod tests {
    ...
    fn valid_documents(agent_name: &str) -> String {
        let provider_id = "github".parse().expect("valid fixture ID");
        let capability_id = "github.pull-request.read"
            .parse()
            .expect("valid fixture ID");
        let provider = ProviderBuilder::new("github").build();
        let capability = CapabilityBuilder::new("github.pull-request.read", provider_id).build();
        let agent = AgentBuilder::new(agent_name)
            .capability(capability_id)
            .provider("github".parse().expect("valid fixture ID"))
            .build();

        [
            serde_yaml::to_string(&provider).expect("provider serializes"),
            serde_yaml::to_string(&capability).expect("capability serializes"),
            serde_yaml::to_string(&agent).expect("agent serializes"),
        ]
        .join("---\n")
    }
    ...
    fn loads_multiple_documents_and_sorts_resources() {
        let mut input = valid_documents("zebra");
        let second = AgentBuilder::new("alpha").build();
        input.push_str("---\n");
        input.push_str(&serde_yaml::to_string(&second).expect("agent serializes"));
        let file = temporary_config(&input).expect("temporary config");
    ...
    fn accepts_json_as_a_yaml_subset() {
        let agent = AgentBuilder::new("reviewer").build();
        let input = serde_json::to_string_pretty(&agent).expect("fixture serializes as JSON");
    ...
    fn rejects_duplicate_resources() {
        let provider = ProviderBuilder::new("github").build();
        let document = serde_yaml::to_string(&provider).expect("fixture serializes");
        let input = format!("{document}---\n{document}");
        let error = LocalCatalog::from_str("duplicate.yaml", &input)
    ...
    fn rejects_missing_capability_references() {
        let agent = AgentBuilder::new("reviewer")
            ...
            .build();
        let input = serde_yaml::to_string(&agent).expect("fixture serializes");
        let error = LocalCatalog::from_str("missing.yaml", &input)
    ...
    fn rejects_missing_provider_references() {
        let capability = CapabilityBuilder::new(
            "github.pull-request.read",
            "github".parse().expect("valid fixture ID"),
        )
        .build();
        let input = serde_yaml::to_string(&capability).expect("fixture serializes");
        let error =
