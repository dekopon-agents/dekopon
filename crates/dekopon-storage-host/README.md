...
Wasmtime-independent broker-owned storage engine for scope-bound provider imports.

`StorageScope::PrivateConversation` retains the original provider/agent/subject/transport/conversation key and authority-bound continuity. `SharedConversation` drops the subject but retains transport and conversation; `Agent` uses provider and agent alone. The latter two use stable continuity in the broker, while every invocation still requires authorization for its own caller. These are storage identities, not gateway conversation-memory scopes. Owner configuration selects `scope: private-conversation|shared-conversation|agent`; the old `namespace: chat` spelling is not accepted. `retention: {mode: keep}` is the default; opt-in `retention: {mode: idle-ttl, idleTtlMs: <positive integer>}` applies to the configured resource family. Conflicting retention for the same provider/scope is rejected at broker startup. Every agent using that provider/scope follows the same configured TTL; losing a caller's Cedar permission revokes access but does not disable that resource family's retention policy.

The broker writes bounded versioned identity metadata and a zero-byte `last-used` marker to the base directory. The marker mtime is the only age source; admitted access, including reads and failed guest operations, refreshes it before guest execution without moving it backward on clock rollback. Denied access and sweeping do not. An untouched legacy private resource has no metadata and is preserved; its first admitted access establishes fresh metadata and starts its age. Missing, corrupt or unknown metadata/policy never authorizes deletion. `StorageHost::sweep(&RetentionPolicies)` examines the bounded namespace tree, skips busy leases and Keep or unexpired resources, and deletes expired resources with the existing namespace lock/lease and quota ledger. `Broker::storage_retention_policies()` supplies the current policy; the host does not schedule sweeps. A shorter TTL makes previously admitted resources eligible at the next sweep; TTL is not an access deadline, and forward wall-clock jumps can expire data early. Partial deletion before metadata removal retains the marker for the next sweep, while a failure during final metadata or lock removal can leave that resource needing explicit operator deletion/reset. Other resources remain available.
