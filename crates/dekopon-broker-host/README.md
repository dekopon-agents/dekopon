...
`BrokerProviderRegistry` compiles configured components once, validates their manifests, and builds deterministic capability routes. Components are compiled concurrently on the blocking pool — Cranelift is a cold start's whole cost — while manifests are consumed in configured order, so the conflict report and the first reported failure do not depend on which compile finished first. Each artifact is read once under a 64 MiB source ceiling and the recorded SHA-256 is of that same buffer, so the published digest describes exactly what was compiled. A managed `dekopon-brokerd` provider lock additionally supplies expected byte length, SHA-256, and provider ID; the host compares all three at this same read/describe boundary before accepting the component, rather than trusting a preflight hash of different bytes. Legacy directly named paths remain supported without an expected lock identity. The registry retains artifact path/size/SHA-256, bounded Wasmtime-visible import/export documentation, and a cloneable process-local metrics handle; broker validation and the provider manager consume loaded-provider metadata. Imports are resolved into one `InstancePre` per provider at load, so a description, invocation, or command run instantiates without rebuilding a linker or re-resolving imports. Every description, invocation, or command run gets a fresh store and component instance. A provider declaring command words must export `run-command` (`argv` plus an optional piped `stdin`) or the legacy `resolve-command` (`argv` alone); the host reads which one the component's type offers at load, refuses a manifest whose words have no callable export, calls `run-command` when both exist, and adapts a legacy answer into the same `CommandRunOutcome` so `BrokerProviderRegistry::run_command` returns one type whichever export served it. A legacy guest never receives `stdin`, by contract. The checked-in `cli-probe` fixture drives `run-command` in this crate's tests — a piped value reaching the guest, and a help page, a usage error, a proposal, and a decline each parsing — and the frozen `provider-v0-2-compat` drives the legacy export; no checked-in component exports both, so the rule that `run-command` wins rests on the lookup order in `dekopon-provider-sdk::host::command_export` rather than on an artifact, and because every checked-in command-word fixture is import-free, the `RunCommandUsedHostImport` tripwire is enforced by the same code path as the describe-mode HTTP state but is not driven by a test.
