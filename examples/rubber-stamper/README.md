...
component, compiles this policy against the world these files declare, and asserts the decision
table both directions, so the walkthrough cannot drift away from the machinery without CI noticing.
...
$EDITOR broker-credentials.yaml          # replace the github_pat_XXXX… placeholder
```
...
| `github_pat_XXXX…` | `broker-credentials.yaml` | the token from step 2 |

Everything else already resolves. Relative paths in both configurations resolve against the
configuration file's own directory, so `../providers/gh-provider.wasm`, `policies.cedar`,
...
> **cpetersen:** approve PR 7 in owner/repo
...
2. **The session.** The agent's `instructions` become the system prompt, and its only tool is the
   ...
   {"number":7,"title":"Cache the provider registry between invocations","state":"open",
    "draft":false,"merged":false,"author":"someone-else","headRef":"cache-registry",
    "headSha":"4f2a91c8e3b7d05a6c1f8e2b9d4a70c3e5f81b26","baseRef":"main",
    "additions":118,"deletions":24,"changedFiles":6,"mergeableState":"clean", ...}

   $ gh pr files 7 -R owner/repo | jq -r '.files[].path'
   crates/dekopon-broker-host/src/lib.rs
   crates/dekopon-broker-host/src/tests.rs
   ...

   $ gh pr review 7 -R owner/repo --approve --body "Read the registry cache change; approving."
   {"reviewId":2418809931,"state":"APPROVED","pullNumber":7,
    "headSha":"4f2a91c8e3b7d05a6c1f8e2b9d4a70c3e5f81b26","author":"xavier",
    "submittedAt":"2026-01-14T09:12:41Z"}
   ```

   Each command word is one `gh.*` capability proposal, not a subprocess: there is no `gh` binary,
   no shell, and no `gh api` — a path-level passthrough would collapse five separately policed
   capabilities into "everything the token can reach", so the builtin refuses it by name.
3. **The answer.** The session's final text goes back to the DM:

   > **dekopond:** Approved PR 7 in owner/repo — review 2418809931. 118 additions across 6 files,
   > head `4f2a91c`.
...
constraint set allows two requests and two methods: one `GET`, then one `POST` carrying a SHA that
was true a moment ago. A retry against the same head converges; a retry after new commits refuses
instead of blessing code nobody read.
...
Records 1 through 5 are the rest of the same session: a `decision` and an `execution` for each of
the two reads, then the `decision` that allowed this approval. Every identifier in the session shares
...
- [`../providers/gh/`](../providers/gh/README.md) — all nineteen `gh` capabilities and why there is no `gh.api.*`.
