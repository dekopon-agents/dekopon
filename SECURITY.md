...
The `0.2.0` development line also contains a privileged asynchronous component host, bounded native HTTP engine, and exact-policy authorization/evidence/audit core. They are exercised only by deterministic tests; no workspace executable authenticates callers or exposes that host path. Findings in trusted-context binding, replay controls, policy matching, audit redaction/integrity, destination validation, DNS/IP controls, bounds, WIT adaptation, Wasmtime isolation, or authorization binding are in scope now.
