...
The `0.2.0` development line also contains a privileged asynchronous component host, bounded native HTTP engine, exact-policy authorization/evidence/audit core, and bounded identity-free Unix client protocol. They are exercised only by deterministic tests; no workspace executable authenticates callers or exposes that host path. Findings in framing/deadlines, socket/server-UID validation, authority omission, trusted-context binding, replay controls, policy matching, audit redaction/integrity, destination validation, DNS/IP controls, bounds, WIT adaptation, Wasmtime isolation, or authorization binding are in scope now.
