...
**D7a reporting-only closure (owner approved).** Retire the web UI's two informational broker operations, `PublishAgentInventory` / `publishAgentInventory` and `PublishModelUsage` / `publishModelUsage`, their client/request constructors and exclusively owned payload/validation/acknowledgement machinery, plus the gateway's startup/periodic inventory publisher and token-usage report channel/accumulator. Prove each removed item has no surviving consumer. This is one atomic D7a unit spanning webui, brokerd, broker-protocol and dekopond, a specific four-runtime-crate sizing exception; no generic gateway or protocol refactor. Corresponding inventory-rendering Rustdoc in dekopon-protocol may change as documentation only, preserving its resource fields, serialization and validation. Under §5's mechanical-consumer rule, the exact stale UI/listener/error-documentation comments in core/src/accept.rs, provider-http/src/lib.rs, broker-host/src/metadata.rs, capability/src/lib.rs, provider-sdk/src/host.rs and telemetry/src/lib.rs may also be corrected (including the three existing test-documentation blocks), with every non-comment byte, API, constant, WIT contract, assertion and runtime behavior preserved. This is documentation closure, not another runtime-crate expansion. Because guest-source comments change, all six pinned deterministic provider builds must be repeated and their components and locks must remain unchanged; do not claim guest source bytes stayed identical. Keep the shared agent ModelUsageObserver/with_usage_observer API and tests (the separately shipped console consumes them), model-client usage data, OpenTelemetry traces and daemon log/trace correlation, all other broker operations, and all credential/HTTP/storage/UID/limit boundaries. Remove report-specific logs/docs with the retired operations, not general telemetry. No replacement no-op report sink, copied UI state store or new exporter.
